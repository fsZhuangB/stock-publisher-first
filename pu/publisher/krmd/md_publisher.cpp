...
#define SHARED_QUEUE_SIZE 65536
const static std::string version = "V0.1.3.20200421";
...
void publish_trade(MdsL2TradeT& trade) {
    ...
    strncpy(data.code, trade.SecurityID, sizeof(trade.SecurityID));
    data.price = trade.TradePrice;
    data.vol = trade.TradeQty;
    data.update_time = trade.TransactTime;
    data.type = TYPE_TRADE;
    data.seq_num = trade.BidApplSeqNum; //special handling
    data.pre_close = static_cast<int>(trade.OfferApplSeqNum); //special handling
    switch (trade.TradeBSFlag)
    {
    case 'B':
...
void publish_snapshot(MdsMktDataSnapshotT& snapshot) {
    ...
    data.seq_num = snapshot.l2Stock.TotalValueTraded;//special purpose:  use seq_num for TotalValueTraded in snapshot_l2 type
...
void publish_index(MdsMktDataSnapshotT& snapshot) {
    ...
    data.seq_num = snapshot.index.TotalValueTraded; //special purpose:  use seq_num for TotalValueTraded
    data.last_price = snapshot.index.LastIdx;
    data.bid_prices[0] = snapshot.index.HighIdx >> 32;//special purpose
    data.bid_prices[1] = snapshot.index.HighIdx & 0xFFFFFFFF;
    data.ask_prices[0] = snapshot.index.LowIdx >> 32;//special purpose
    data.ask_prices[1] = snapshot.index.LowIdx & 0xFFFFFFFF;
...
MdsApiSample_HandleMsg(MdsApiSessionInfoT *pSessionInfo,
    ...
    case MDS_MSGTYPE_L2_TRADE:
        ...
        if (pRspMsg->mktDataSnapshot.head.exchId == MDS_EXCH_SZSE
            || pRspMsg->mktDataSnapshot.head.exchId == MDS_EXCH_SSE) {
...
void start_md(Config& config){
    fprintf(stderr, "stock_publisher version: %s\n", version.c_str());
    ...
    printf("MdsApi_InitAllByConvention ok\n");

    sleep(2);
    
    const string& routeKey = config.routeKey();
    if (routeKey.length() > 0) {
        printf("MdsApiSample_ResubscribeByCodePostfix routekey\n");        
    ...
    if (routeKeySH.length() > 0) {
        printf("MdsApiSample_ResubscribeByCodePostfix sh routekey\n");                
        if (! MdsApiSample_ResubscribeByCodePostfix(&cliEnv.tcpChannel,
